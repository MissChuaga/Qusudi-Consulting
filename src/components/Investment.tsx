  ...
  const packages = [
    ...
    },
    {
      ...
      features: [
        ...
        "Roadmap with 6–12 month priorities",
        "2 advisory calls per week",
    ...
    },
    {
      ...
      features: [
        "Complete GTM Audit",
        "6–12 month prioritized roadmap",
        "Embedded GTM operator",
        "Growth Modeling",
        "3+ calls per week",
        "Unlimited async access"
      ],
    ...
    },
    {
      ...
      features: [
        ...
        "Strategic advisor / fractional leader",
        "8–12 hours/month guidance",
        "Ongoing advisory and leadership",
        "On-demand support"
      ],
      highlight: false
    }
    ...
    <Section id="investment">
      <div className="text-center max-w-3xl mx-auto mb-16">
        ...
        <p className="text-lg text-gray-600">
          Choose the engagement level that matches your needs. From diagnostics to full embedded leadership.
        </p>
      </div>
        ...
        {packages.map((pkg, index) => (
          <div 
            ...
            className={`flex flex-col rounded-2xl p-6 transition-all duration-300 ${
              pkg.highlight 
                ? 'bg-gray-900 text-white shadow-xl scale-105 z-10 ring-2 ring-blue-500' 
                : 'bg-white text-gray-900 border border-gray-200 hover:border-blue-300 hover:shadow-lg'
            }`}
          >
            <div className="mb-6">
              ...
              <p className={`text-sm mb-4 min-h-[2.5rem] ${
                pkg.highlight ? 'text-gray-400' : 'text-gray-500'
              }`}>
                {pkg.subtitle}
              </p>
              <div className="flex items-baseline">
                {pkg.prefix && (
                  <span className={`text-xs mr-1 ${pkg.highlight ? 'text-gray-400' : 'text-gray-500'}`}>
                    {pkg.prefix}
                  </span>
                )}
                <span className="text-3xl font-bold">${pkg.price}</span>
                {pkg.period && (
                  <span className={`text-sm ml-1 ${pkg.highlight ? 'text-gray-400' : 'text-gray-500'}`}>
                    {pkg.period}
                  </span>
                )}
              </div>
